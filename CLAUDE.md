...
- [Platform/A2v10.Metadata/CLAUDE.md](Platform/A2v10.Metadata/CLAUDE.md) — Declarations: rules by kind, layered by "mine wins" · System endpoints: behaviour is a type, not a kind · Forms: whole or nothing · Commands: a derived set, plus what the endpoint declares · Members: a form node shows members, not columns · Filters: a namespace on the shape, referenced by the form · Posting by procedure: a box with two ports · Transactions: a projection of `post`, not a screen · Print forms: paper under one act · Autonums: a column and a key, never a trait · Chart of accounts: a kind with a natural key and a seed file · States: a kind for the set, a type for the column · Colour: a property of the row, never of the reference · Cases: the lifecycle the user owns · Registers: what holds over an interval, written as two rows · Links: part and belonging, and one naming rule · Two files in one folder: data from the shape, screen from model.json · Names: what every place a name lands in accepts · Stamps: typed by what a walk asks, written by name · Logins: a view to resolve by, a module to choose in · Seed: what the database is told about itself · Validation: the endpoint assembled, never the file parsed
- [Extensions/A2v10.Scheduling/CLAUDE.md](Extensions/A2v10.Scheduling/CLAUDE.md) — The command queue is FIFO, so "after a batch" is just a later command · Run time: `UtcDateStarted`, never `LockDate` · Ordering inside one queue call: not built
...
- [Tools/A2v10.Cli/CLAUDE.md](Tools/A2v10.Cli/CLAUDE.md) — The `a2` CLI: shape of the command tree · `meta validate`: one endpoint, no switches, all checks listed
